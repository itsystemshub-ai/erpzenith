    ...
    <div className="flex flex-col min-h-screen">
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <GlassCard className="w-full max-w-lg p-6 space-y-3 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-headline font-bold text-on-surface">Nuevo Empleado</h3>
            <div className="grid grid-cols-2 gap-3">
              {([
                ...
                { k: 'salarioUSD',   l: 'Salario USD' },
                { k: 'fechaIngreso', l: 'Fecha Ingreso' },
                { k: 'telefono',     l: 'Teléfono' },
                { k: 'direccion',    l: 'Dirección' },
              ] as { k: keyof typeof EMPTY_FORM; l: string }[]).map(({ k, l }) => (
                <div key={k} className={k === 'direccion' ? 'col-span-2' : ''}>
                  <label className={labelClass}>{l}</label>
                  <input type={k === 'fechaIngreso' ? 'date' : k === 'salarioUSD' ? 'number' : 'text'}
                    value={form[k]} onChange={setF(k)} className={inputClass} />
                </div>
              ))}
              <div>
            ...
            {/* Estado y Municipio Venezuela */}
            <GeoSelector
              region=""
              estado={form.estadoGeo}
